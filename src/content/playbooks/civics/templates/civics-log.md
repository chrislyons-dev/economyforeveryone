---
title: 'Civics Log (Consistency > Intensity)'
last_updated: 2026-03-01
status: draft
lane: Civics
tags:
  - civics
  - playbook
  - template
---

# Civics Log (Consistency > Intensity)

**Why:** memory beats motivation. This keeps you steady without spiraling.

## This week’s lane (choose one)

- [ ] Voting
- [ ] Policy
- [ ] Mutual aid
- [ ] Info hygiene

## This week’s “one steady thing”

- [ ] One Ask sent
- [ ] One meeting attended/watched
- [ ] One micro-team touch
- [ ] One comment submitted
- [ ] One correction done calmly (or one inflammatory post not shared)

## Actions (copy/paste rows)

| Date       | Level               | Target      | Playbook                    | One Ask | Outcome metric | Next step | Follow-up date | Notes     |
| ---------- | ------------------- | ----------- | --------------------------- | ------- | -------------- | --------- | -------------- | --------- |
| YYYY-MM-DD | local/state/federal | name/office | one-ask / meeting / comment | text    | metric         | next step | date           | 1–2 lines |

## Receipts stub (optional)

- Sources checked:
  - [ ] Primary doc
  - [ ] Reputable reporting
  - [ ] Official data
- What’s confirmed:
- What’s plausible:
- What’s unknown:
