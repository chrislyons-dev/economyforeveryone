...
- [AI and Personalized Pricing \& Steering in Essentials](#ai-and-personalized-pricing--steering-in-essentials)
  ...
  - [15. Required checks](#15-required-checks)
  - [16. What to do](#16-what-to-do)
  - [17. What good looks like](#17-what-good-looks-like)
...
A renter in Charlotte is deciding between two apartment complexes. Both are managed by large regional landlords. Both list similar rents. She picks the one closer to work. What she cannot see is that both landlords use the same software -- RealPage's YieldStar -- and that software has been pooling non-public occupancy and lease-rate data from competing landlords across her submarket to set daily price recommendations. The listed rent she's comparing is not the product of two landlords competing for her business. It's the product of a single algorithm coordinating across them. She has no way to know this. No disclosure is required. By one academic estimate, her rent is about $70/month higher than it would be under competitive pricing -- but the mechanism is what the evidence firmly supports: coordination, invisibility, and no way to contest. She signs the lease.

---

## 2. What's happening

Pricing algorithms in essential markets -- rent, insurance, credit, healthcare -- are shifting extraction toward consumers who cannot exit. The efficiency gains from AI price-setting accrue to the firms that deploy it. The consumers absorbing higher prices or worse terms have no meaningful way to detect, contest, or escape the algorithm.

### What the helpful version looks like

Algorithmic pricing can genuinely help consumers when competition is real: insurance telematics programs that lower premiums for safe drivers, personalized loan offers that surface better terms for people with thin credit files, dynamic pricing that signals excess supply. The difference between "pricing that helps" and "pricing that extracts" is not the algorithm -- it is the market structure. In competitive markets with low switching costs and good information, algorithmic precision can produce consumer surplus. In captive markets -- rent, mandatory insurance, employer health plans -- the same precision becomes extraction at scale. The welfare-improving case for personalized pricing fails exactly where captivity is present. [supports the captivity condition analysis in Section 5]

**Why it usually doesn't happen:** Because the markets where AI pricing is most aggressive are precisely the captive ones. The competitive pressure that should pass efficiency gains to consumers does not exist in a housing market where two coordinated landlords are the only options, or in a health plan market where open enrollment is once a year. The algorithm is not the problem. The absence of a competitive floor is.

**What makes the helpful version more likely:** Portability and interoperability requirements that lower switching costs; ban on using non-public competitor data in mandatory-market pricing models (the RealPage settlement principle extended); audit rights so regulators can verify whether efficiency gains are flowing to consumers or to margin.

---

## 3. Why it's happening -- the mechanisms

This case involves two rails. Conflating them obscures the analysis.

- **Rail A: coordination at scale.** Competing firms share non-public pricing data through a common algorithm. The harm is market-level: the competitive price signal is replaced by a coordinated one. (Primary example: RealPage rent-setting.)
- **Rail B: individual targeting and steering.** A consumer is assigned to a price tier or routing path based on signals that act as proxies for characteristics the firm cannot (or should not) use directly. The harm is individual-level: unequal prices, unequal access, opaque recourse.

The mechanisms below are the reusable patterns that make those rails possible. Each mechanism includes **Mechanism Modules** (numbered sub-items) that you can reuse across case studies.

**Accountability laundering (Rail B).** When a consumer is overpriced, steered, or denied on an essential market decision, the accountability chain runs backward through data broker, model vendor, and institution, with each layer disclaiming the next. The broker sold a ZIP code; the model weighted it; the insurer acted on the score. No entity is legally required to explain the full chain to the affected consumer. The fix requires assigned decision ownership: the institution that acts on the output owns accountability for the output, regardless of upstream data provenance. This is the same accountability diffusion documented in claims eligibility, where the complexity of the UM vendor stack shields the harm from any single accountable owner.

### Mechanism 1: Contestability collapse

When systems become too fast/cheap/opaque to overrule, "human review" becomes a rubber stamp. Leverage quietly moves from people to institutions.
...
| Element            | Status                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Notice**         | Partial and jurisdiction-dependent. New York Algorithmic Pricing Disclosure Act (effective November 10, 2025) requires disclosure when a price is set by algorithm using personal data. No federal analog. Healthcare PA decisions: no federal requirement to disclose AI involvement. [confirmed, PP-004, PP-011]                                                                                                                |
| **Reason**         | Incomplete. CFPB Circular 2023-03 requires specific adverse action reasons for AI credit denials; enforcement uncertain as of March 2026. No equivalent standard for insurance pricing or healthcare PA routing under federal law. Where reasons are provided, they often reference criteria categories rather than specific model inputs. [confirmed for gap, PP-003]                                                            |
| **Appeal**         | Nominally present; structurally deterred. Healthcare PA: appeal pathway exists; overturn rates are significant when pursued; most patients do not pursue. Credit: adverse action notice required; appeal path through creditor, not through a neutral party. Rent: no appeal mechanism for a coordinated price. [confirmed for structural deterrence, PP-011; confirmed for absence in rent, PP-001]                              |
| **Records**        | Not accessible to consumers in any system reviewed. Pricing inputs, model version, output price, and benchmark comparison are not available to the consumer on demand. Regulator access is jurisdiction-dependent and generally not operationalized. [confirmed for gap, Section 9]                                                                                                                                               |
| **Human override** | Present for some decisions; structurally marginal for others. Healthcare PA: physician-to-physician review available on appeal; not required before initial AI denial. Rent: landlords could technically reject RealPage recommendations; DOJ complaint documents acceptance as the structural default. Credit: human review available at creditor discretion; not required for AI-driven adverse actions. [confirmed, Section 8] |

**Finding:** Human Command is not present in any essential market reviewed at a level that meets the minimum floor. The most proximate implementation is CFPB Circular 2023-03 (credit reason codes) and New York's disclosure law. Both are partial and one is of uncertain enforcement status.
...
The rubber stamp in this domain is not a reviewer clicking through denials too fast. It is a landlord or insurer accepting an algorithmic output because the system is designed to make acceptance the path of least resistance, and rejection would require overriding a recommendation they cannot independently verify.
...
[ANCHOR NEEDED] Pull any measured rates that exist for these domains (RealPage/ride-hail/prior auth/insurance pricing) and cite them.
...
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Rent             | No for most lower-income renters in affected submarkets. Geographic mobility requires 2-4 months of rent in direct switching costs; coordinated pricing covers large shares of available units in concentrated submarkets. [plausible-strong, PP-012; confirmed for switching cost structure] | Switching costs (high) + information asymmetry (coordination invisible) = 2 conditions.                                             |
| Auto insurance   | Partial. Comparison-shopping is possible; shopping friction is real; switching requires coordination of new and old policies; model similarity across insurers may limit the benefit of switching. [plausible, PP-007]                                                                        | Regulatory requirement (49 states) + information asymmetry (model invisible) = 2 conditions.                                        |
| Health insurance | No mid-year. Plan-year lock-in is confirmed. Annual enrollment permits switching; network differences may constrain effective choice even then. [confirmed, Section 5]                                                                                                                        | Switching costs (plan-year lock-in) + network lock-in (provider network) + information asymmetry (AI not disclosed) = 3 conditions. |
| Credit           | Partial. Multiple lenders available; inquiry penalty for shopping; model similarity across lenders uncertain. [plausible]                                                                                                                                                                     | Information asymmetry (model invisible) + moderate switching cost (inquiry penalty) = 2 conditions.                                 |
| Gig labor        | No for platform-dependent workers. No rating portability across platforms; leaving a platform means losing accumulated review history. [confirmed, PP-005, PP-006]                                                                                                                            | Network lock-in (ratings not portable) + information asymmetry (formula invisible) = 2 conditions.                                  |

The switching costs are not abstract. Switching health insurers mid-year typically requires a qualifying life event (job change, marriage, birth). Switching landlords means deposit loss, moving costs, and the risk of landing in a worse market. Switching auto insurers may trigger a lapse-in-coverage penalty on your next quote. In each case, the "choice" to stay is not a preference -- it is an exit barrier that the algorithm's owner faces no pressure to lower.

**Finding:** Exit is not realistic in any essential market reviewed under the captivity definition. This elevates the governance burden: where exit fails, disclosure, audit rights, and contestability are not optional governance improvements -- they are the only available mechanism to prevent extraction from becoming the structural equilibrium.
...
[ANCHOR NEEDED] Add at least one concrete example (insurance/credit/ride-hail) showing "offer provenance" in practice.
...
In pricing and steering contexts, "identity" does not mean authentication -- it means segmentation. The algorithm assigns a consumer to a risk bucket, a price tier, or a routing path based on signals that serve as proxies for characteristics the firm cannot or legally should not use directly.

The mechanism is documented in Section 4, Walkthrough B: credit scores, ZIP codes, and external behavioral data enter the model. These signals correlate with race and income not as intentional design choices, but as statistical artifacts of historical segregation and wealth inequality. The model optimizes for predictive accuracy using the signals available. The outcome is disparate: ZIP codes with predominantly Black residents face 60% higher home insurance premiums than predominantly white ZIP codes after controlling for population density. [confirmed directionally, Consumer Federation of America, cited in PP-007]

This is proxy discrimination. The consumer was not segmented by race. The consumer was segmented by ZIP code, which functions as a racial proxy in the model's optimization. The result is indistinguishable from direct discrimination in its distributional effect.

##### Confirmed enforcement anchor

The Massachusetts AG settlement (July 2025) is the confirmed enforcement anchor for this mechanism in an AI underwriting context. A student loan company's AI underwriting models produced unlawful disparate impact based on race and immigration status. The AG settled. [confirmed, PP-008]

CFPB Circular 2023-03 (September 2023) provides the structural context: AI credit denials must give specific, accurate reasons. Generic black-box notices that do not specify which factors triggered a denial violate ECOA. Before the circular, industry practice was generic notices that consumers could not use to contest a decision. [confirmed, PP-003]

##### Identity integrity: synthetic identities and verification failure

One underappreciated interaction: the same pricing and eligibility models that use identity signals to segment consumers are vulnerable to synthetic identity fraud -- where the identity signals are fabricated. FinCEN Advisory FIN-2023-A003 (November 2023) documented that AI-generated deepfake images, audio, and documents are being used to defeat remote identity verification at financial institutions. Standard liveness detection may be insufficient. [confirmed for mechanism and regulatory recognition; scale of deepfake-specific losses not quantified in advisory, S-08]
...
- **Contestable segmentation decisions:** A consumer who is assigned to a price tier or routing path must be able to contest the assignment with specific, actionable information about the basis.
...
| **Can the affected consumer contest it using the log?**                   | No. A consumer cannot contest a pricing decision using information they cannot access. The most proximate right is CFPB Circular 2023-03's requirement for specific credit denial reasons -- which provides a basis for contesting a denial but does not require disclosure of model inputs, weights, or the benchmark rate the consumer would have received without personalization.                                                                                                                 |

**Finding:** Asymmetric logs are the structural norm. Institutions have the pricing decision records; consumers do not. This is the precise A4 gap identified in the coverage map. Without log access, the audit and contestability minimums in Boxes 1 and 2 cannot be operationalized even where they formally exist. The Section 9 governance framework (audit log retention as a minimum-floor requirement) addresses this gap; it is not currently required anywhere in the US for pricing algorithms.
...
[ANCHOR NEEDED] Add concrete retention windows from at least one domain (insurance claim files, credit adverse action disputes, prior auth).
...
Track whether the system is effectively acting as a regulator/judge:
...
[ANCHOR NEEDED] Add measured "appeal/reversal" or "routing error" rates where available.
...
[ANCHOR NEEDED] Add one domain example where corroboration is already required (or where the absence has caused harm).
...
[ANCHOR NEEDED] Not primary in this case study. If you want this mechanism here, adapt from the IT ladder collapse case study: periodic manual runs, sampled overrides, and training rotations.

#### Module 6.2: Learning-work quotas

[ANCHOR NEEDED] Not primary in this case study. If you want this mechanism here, define protected "judgment reps" in the relevant domain (claims adjusters, underwriters, clinicians, regulators).
...
Pricing and steering algorithms do not operate in a vacuum. They operate through gates. In the essential markets documented here, the relevant gates are:

- **Platform gate (RealPage):** A single software vendor aggregated non-public pricing data from competing landlords across a submarket. Landlords who wanted the coordination advantage had to supply their own competitive data to get it. That is not a pricing tool -- it is a cartel-coordination infrastructure. The power lives with whoever controls the data pool. [confirmed, PP-001]
- **Insurer gate:** In health, auto, and home insurance, the insurer controls the eligibility model, the rate-setting model, and the claims-routing model. No public interface exists for comparing algorithmic inputs across insurers. A consumer denied or overpriced by one insurer's model has no mechanism to verify whether an alternative insurer runs a materially different model. [confirmed for opacity, PP-003, PP-007]
- **Data broker gate:** Credit scores, ZIP codes, and behavioral data enter insurance and lending models from external data brokers. The broker is invisible to the consumer but is the upstream source of the proxy discrimination harm. A consumer cannot dispute their ZIP code's correlation with race in a pricing model because they do not know the model uses it, cannot see the weight assigned, and cannot access the broker's dataset. [confirmed for mechanism, PP-002, PP-003]

##### How dependence blocks shared gains

The efficiency-improvement argument for algorithmic pricing assumes competitive pressure will pass gains to consumers. In captive essential markets, that condition does not hold.

A renter facing two RealPage-coordinated landlords has no non-coordinated alternative in her submarket. The software's efficiency improvement -- faster, more granular price-setting -- is captured entirely as higher rent because there is no competitive floor to push prices down. The captivity definition in Section 5 is the load-bearing variable: when two or more captivity conditions are simultaneously present (switching costs, network lock-in, coverage requirements, information asymmetry), the firm's optimization objective shifts from output maximization to reservation-price extraction. The welfare-improving price discrimination case fails in exactly this condition. [plausible-strong, consistent with Section 5 analysis and PP-D01 disconfirming evidence assessment]

For the insurer and data broker gates, dependence is compounded by invisibility. The consumer cannot identify the bottleneck, which means they cannot price-shop around it, advocate against it, or detect when the efficiency gain has been captured as margin.

**Dependency amplifier.** In each essential market documented here, the institution's dependence on a single algorithmic vendor amplifies the pricing harm. A landlord who cannot operate without RealPage's coordination signal, an insurer whose underwriting depends on a single credit-score model, a healthcare system whose PA routing depends on one AI vendor: in each case, a model update, outage, or policy change becomes a system-wide shock with no fallback path. The captive consumer (Section 5) and the vendor's operational indispensability compound: the institution cannot exit either without significant cost, which means the vendor's leverage persists through every contract renewal. [ANCHOR NEEDED: documented vendor update or outage producing pricing or claim-processing disruption in reviewed markets.] This is the same single-point-of-failure dynamic documented in surveillance infrastructure, where vendor lock-in converts procurement into permanent coercion capacity.

**Upstream capture via data brokers.** The data broker layer in this case is also an upstream extraction problem: location, transaction, and behavioral data that feeds pricing models was generated by individuals who received no compensation for supplying it. As AI models are trained on aggregated consumer behavioral data, the value extraction extends upstream -- the price signal trained on your behavior gets sold back to the institution that prices you. Opt-out mechanisms exist in theory (CCPA, CPRA in California); in practice, the broker ecosystem is not auditable by individual consumers. [plausible -- confirmed for mechanism; individual-level income impact of behavioral data extraction is not independently measured.] This is the same upstream capture mechanism documented in the content flood case, where creator work becomes training input without consent or compensation.
...
- **Portability:** Require that pricing history and model inputs be portable to regulators on demand. Prevents firms from arguing they cannot reconstruct pricing decisions after the fact.
...
[ANCHOR NEEDED] Add one concrete example of a "kill switch" requirement (or absence) in a regulated market.
...
[ANCHOR NEEDED] Add one example of rate-limit style governance (financial trading controls, medical device safety controls, etc.).

## 4. Examples and walkthroughs

### 4.A Housing -- algorithmic coordination (Rail A)

**Step 1 -- Data capture [confirmed]:** RealPage's YieldStar required participating landlords to upload current lease rates, occupancy rates, and renewal offers. This data was non-public and competitively sensitive. Landlords shared it as a condition of using the software. (DOJ complaint, August 2024)

**Step 2 -- Model generates price recommendation [confirmed for mechanism; plausible-strong for "above what individual landlord would set"]:** YieldStar aggregated the data across competing landlords in the same submarket and generated daily rent recommendations. Because the model incorporated non-public competitor data, it could recommend rates above what any individual landlord would set based on their own occupancy alone. This is the DOJ's theory of harm; it has not been independently modeled here.

**Step 3 -- Why the renter cannot detect it [confirmed]:** The listed rent is visible. The coordination behind it is not. No disclosure was required before New York's November 2025 law -- and that law covers a different mechanism (individual behavioral pricing), not algorithmic market coordination.

**Step 4 -- Why the renter cannot exit [plausible-strong for penetration; confirmed for switching cost structure]:** DOJ complaint documents over 4 million units using the software. Some estimates suggest 1 in 4 rentals used some form of algorithmic pricing system. Exiting to a non-coordinated landlord required geographic mobility most renters do not have. Switching costs run 2-4 months of rent in direct expenses.

**Step 5 -- How the margin is captured [confirmed for mechanism; plausible-strong for dollar figure]:** Higher coordinated rents translate directly to higher revenue per unit. Academic research cited in the Washington Post (2025) estimates algorithmic pricing added approximately $70/month or 4% per affected unit, totaling roughly $3.8 billion in added renter cost in 2023. The primary academic paper was not independently read for this research file. Do not treat this figure as confirmed. The mechanism is confirmed by DOJ complaint and settlement.

---

### 4.B Insurance and credit -- proxy discrimination and opacity (Rail B)

**Step 1 -- Data capture [confirmed for credit score and ZIP code inputs; plausible for expanded behavioral data]:** Insurers and lenders ingest applicant data including credit score, ZIP code, employment history, and -- in some systems -- external behavioral data. These signals feed a proprietary model that generates a risk score or premium. (NY DFS Circular 2024-7; NAIC Model Bulletin 2023; FTC surveillance pricing study, January 2025)

**Step 2 -- Model changes the price or offer [confirmed for racial correlation; plausible for "without human review" as standard practice]:** Credit scores and ZIP codes correlate with race and income -- not as intentional design choices, but as statistical artifacts of historical segregation and wealth inequality. The model optimizes for predictive accuracy using these signals, generating disparate outcomes for protected classes even without discriminatory intent. Missouri and Texas state insurance department studies found that a consumer's race was the most predictive factor in insurance scoring. ZIP codes with predominantly Black residents face 60% higher home insurance premiums than predominantly white ZIP codes after controlling for population density (Consumer Federation of America).

**Step 3 -- Why the person cannot detect it [confirmed]:** Before CFPB Circular 2023-03 (September 2023), AI credit denials were routinely issued with generic notices that did not specify which factors triggered the denial. Even with specific reasons, the applicant cannot see model weights or input data. A consumer in a ZIP code that generates a higher insurance premium cannot see that their ZIP code is the primary driver, or that it functions as a racial proxy in the model.

**Step 4 -- Why the person cannot exit [confirmed for mandatory insurance structure; plausible for "similar model at alternative lender"]:** Auto insurance is legally required in 49 states. Health insurance is functionally required for access to non-emergency care at non-catastrophic cost. A consumer who receives an unfavorable algorithmic assessment cannot exit the need for the product. Alternative providers may run similar models with similar biases.

**Step 5 -- How the margin is captured [plausible-strong; causal separation from legitimate risk factors is the contested gap]:** Higher premiums for lower-income and minority consumers in flagged ZIP codes generate additional revenue not explained by actuarial risk differential. The residual after legitimate risk factors are controlled for is the proxy discrimination harm. Direct margin capture attributable to algorithmic adoption -- as distinct from other pricing factors -- is not independently quantified in available peer-reviewed literature. This is the strongest methodological weakness in the insurance pricing claim. See PP-U02.

---

### 4.C Healthcare -- prior authorization as a route to cheaper (Rail B)

**Step 1 -- Data capture [plausible-strong]:** When a provider submits a prior authorization request, the insurer's AI system ingests clinical codes, diagnosis history, patient demographics, and plan type. Medicare Advantage plans run AI-driven PA review systems. (Senate PSI "Refusal of Recovery" report, October 2024; AMA 2024 survey)

**Step 2 -- Model changes the route [plausible-strong for mechanism and speed; plausible for "optimizes for cost, not appropriateness"]:** The AI system flags the request for denial or modification -- recommending a lower-cost care setting or denying the requested treatment and requiring appeal. The Senate PSI documented that some denials were issued in less time than a physician would need to read the file. The claim that the model optimizes for cost rather than clinical appropriateness is the theory of harm; clinical appropriateness validation of AI PA decisions has not been independently established.

**Step 3 -- Why the patient cannot detect it [confirmed for no AI disclosure requirement; plausible for patient unawareness of appeal rights]:** The patient receives a denial citing medical necessity criteria. No federal requirement exists to disclose whether AI was used in the PA decision. Patients often do not know they have appeal rights, or that appeal success rates are significant when pursued.

**Step 4 -- Why the patient cannot exit [confirmed for plan-year lock-in; plausible for out-of-network cost deterrent]:** Health insurance is plan-year locked. The patient cannot switch to a plan without AI-driven PA mid-year. Out-of-network care for a denied treatment incurs materially higher cost-sharing.

**Step 5 -- How the margin is captured [plausible-strong for mechanism; plausible-strong for AHA dollar figure -- AHA's own reported estimate, not an independent audit]:** When a denial results in the patient accepting a lower-cost care setting, the insurer avoids paying the higher-cost claim. AHA (2024) attributes $19.7 billion per year in costs to hospitals from prior authorization delays -- this is a hospital cost figure, not a direct measure of patient harm. The mechanism connecting it to patient impact: AHA reports that average Medicare Advantage patient length of stay before post-acute discharge doubled vs. Traditional Medicare from 2019 to 2024, consistent with PA delays forcing longer hospital stays and care setting mismatches than medically appropriate. The patient harm is the downstream consequence of that misrouting, not what the $19.7B figure itself measures. The AHA is an industry advocacy organization; treat the figure as plausible-strong without independent replication.

**Auto-steering is action, not suggestion.** In healthcare prior authorization (Rail B), the routing decision does not require the patient to know a recommendation occurred. The denial is the action; the patient's contest burden starts from zero information. In ride-hail and gig work, the deactivation is immediate income termination, not a recommendation pending review. These are not suggestions that a human can override before they take effect -- they are automated enforcement at scale. The pre-action floor right (corroboration, notice, human override) is not a due-process formality; it is the only point in the workflow where the error cost can be returned to the institution instead of being borne entirely by the person the action affects. This is the same "action without adjudication" pattern documented in surveillance coercion and claims eligibility, applied to consumer markets.
...
| --------------------------------- | ------------------------------------------------------------------ | --------------------------------------- | -------------------------------------------- | -------------------------------------- | ---------------------------------------- |
| Switching costs (money + time)    | High [confirmed]                                                   | Moderate [plausible]                    | High (annual lock-in) [confirmed]            | Moderate (inquiry penalty) [plausible] | Rating loss [confirmed]                  |
| Network lock-in                   | Partial                                                            | Partial                                 | High (provider network) [confirmed]          | Moderate                               | High (no rating portability) [confirmed] |
| Regulatory / coverage requirement | Economic non-option for lower-income households [plausible-strong] | Legally required, 49 states [confirmed] | Functionally required [plausible]            | N/A                                    | N/A                                      |
| Information asymmetry             | High (coordination invisible) [confirmed]                          | High (model invisible) [confirmed]      | High (no AI disclosure required) [confirmed] | High (weights invisible) [confirmed]   | High (formula invisible) [confirmed]     |

The welfare-improving price discrimination argument (see Section 6, disconfirming evidence) depends on competitive pressure that does not exist in captive essential markets. This distinction -- not whether algorithms are used -- is the load-bearing variable for whether personalized pricing helps or harms consumers in a given market.
...
### Plausible-strong (do not assert as confirmed)
...
### Plausible (single source or prospective)
...
- Net welfare effect of personalized pricing for lower-income consumers in rent, insurance, and healthcare specifically (PP-U01)
- What percentage of insurance premium disparity reflects proxy discrimination vs. legitimate actuarial factors in current algorithmic systems (PP-U02)
- Whether RealPage settlement produces measurable rent reductions (PP-U03)
- FTC and CFPB enforcement posture on surveillance pricing and AI credit guidance under current administration (PP-U04)
- Whether Huskey disparate impact theory survives appellate review (PP-U05)

---

## 7. Disconfirming evidence

These arguments are the industry's strongest. They are not dismissed; they apply to specific conditions that differ from the captive essential market context.

**PP-D01 -- The welfare-improving price discrimination argument:** Standard price discrimination theory holds that personalized pricing can expand output and lower effective prices for price-sensitive consumers in competitive markets. A firm that can charge different prices to different consumers can serve lower-income consumers at prices they could afford, while charging higher prices to consumers with higher willingness to pay. (Mercatus Center, 2025; CCIA, "Personalized Discounts, Public Gains")

Assessment: This argument is strongest in optional markets with genuine competition and no captivity. It is weakest in essentials (rent, insurance, healthcare) because: (a) there is no outside option at a lower uniform price; (b) captivity allows extraction up to the full reservation price; (c) the "lower price for the poor" story requires the firm to actually offer those lower prices -- and in captive markets there is no competitive pressure to do so. The welfare-improvement case assumes the firm's optimization objective is output maximization. In captive markets the objective is profit maximization at reservation price. The mechanism by which lower-income consumers benefit does not exist when they cannot leave.

**PP-D02 -- Airline dynamic pricing shows mixed welfare effects:** Williams (Econometrica, 2022) found that in single-carrier markets, dynamic pricing improves aggregate welfare vs. uniform pricing. Competing research found that in multi-carrier competitive markets, dynamic pricing reduces total welfare by 2.2% vs. uniform pricing. Airline welfare effects depend critically on market structure. Airline is also not an essentials market in the E4E sense; these findings do not generalize to rent, insurance, or healthcare.

**PP-D03 -- Surge pricing increases supply:** Uber's own research documents that surge pricing increases driver availability during high-demand periods, reducing wait times. The counterfactual (no surge) produces rationing through wait rather than price. Assessment: Valid for the supply-response mechanism. The Oxford 2025 study documents that Uber's 2023 shift to "upfront pricing" -- which decouples the consumer fare from driver pay -- captured the efficiency gain for Uber rather than distributing it to either side. Supply-response supports surge in competitive ride-hail; it does not support opaque decoupling of consumer price from driver pay.
...
| Human-on-the-loop | Nominal. Landlords technically reviewed RealPage recommendations but the structure made rejection the exception. Gig workers can reject individual jobs but cannot see the algorithm or modify pay.                                                                                                                             |
...
The safety gap here is upstream: these systems were deployed in legally and socially sensitive domains without pre-deployment validation that the mechanism they operate on is lawful. The RealPage case is not a story of a validated system that drifted. It is a story of a system whose core operating mechanism -- pooling non-public competitor pricing data -- was never tested against antitrust law before deployment at scale.
...
| ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1. Did prices fall, especially big costs?              | No for rent, insurance (directional), ride-hail. Unknown for credit.                                                                                                                                                                                                                            | Rent: algorithmic coordination added ~$70/month in affected units (plausible-strong, PP-012). Ride-hail: passengers paid more after 2023 algorithm change (confirmed, PP-005). Insurance: premiums rose for lower-income and minority consumers (confirmed directionally; margin capture not isolated to AI adoption, PP-U02). |
...
| 3. Did service quality improve?                        | No for ride-hail (drivers' real earnings fell, confirmed PP-005). Plausible harm for healthcare (PA delays associated with longer hospital stays and care setting mismatches, plausible-strong, PP-011). Unknown for rent and insurance.                                                        | For gig labor, driver income is a service quality dimension for workers.                                                                                                                                                                                                                                                       |
...
**Test result:** Fails for all essential markets reviewed. The failure is not uniform. Ride-hail has direct confirmed bilateral extraction. Rent has confirmed mechanism with plausible-strong magnitude. Insurance has confirmed proxy discrimination harm with unquantified margin capture attributable to algorithmic adoption specifically. Healthcare routing has a documented mechanism; AI-specific dollar harm is not separated from all prior auth activity. Do not overstate the healthcare finding.

---

## 15. Required checks

Quick references to the reusable Mechanism Modules (see Section 3):

- **Module 1.1** Human Command (minimum floor)
- **Module 2.1** Exit test
- **Module 4.1** Audit/log checklist
- **Module 7.2** Shared gains test (also in Section 11)

## 16. What to do

**One personal action:**

If you are comparison-shopping rent, insurance, or a loan: ask, in writing, whether the price you received is individualized using personal data, and what data was used. In New York, businesses using algorithmic pricing must disclose it. In other states, there is no obligation to tell you -- but asking creates a record and may prompt disclosure. For healthcare prior authorization denials, always ask for the specific clinical criteria used and whether AI was involved; you have the right to appeal and overturn rates are significant when appeals are pursued.
...
A pricing algorithm in an essential market earns its place by demonstrating that it improves access and reduces costs -- not by asserting that it does. The minimum conditions:

- **Disclosure:** consumers know when a price is individually set using their data.
- **No cartel substitution:** competitors do not share non-public pricing data through a common model. If "competition" is really one shared pricing brain, it's not competition -- it's coordination with extra steps.
...
What this looks like in practice today: there is no mechanism for a renter to ask "what would this unit cost if the landlord weren't using RealPage?" There is no mechanism for an insurance applicant to ask "what rate would I receive if my ZIP code wasn't in the model?" The benchmark doesn't exist. The audit doesn't happen. The lever proposed here would create both -- which is why it is absent from current standard contracts and why it has to be required, not requested.

The alternative is not no pricing algorithms. It is pricing algorithms that operate in the open, whose inputs are defensible, and whose distributional effects are visible. Right now, for the essential markets in this case, none of those conditions are met.

---

## Research gaps

- `[RESEARCH GAP: Net welfare direction for lower-income consumers in specific essential markets under algorithmic pricing vs. uniform pricing -- the disconfirming case is theoretically coherent but empirically undocumented in captive markets (PP-U01).]`
- `[RESEARCH GAP: Causal separation of proxy discrimination from legitimate actuarial factors in current algorithmic insurance pricing at national scale (PP-U02). This is the strongest methodological weakness in the insurance pricing claim.]`
- `[RESEARCH GAP: Primary academic paper underlying the $70/month / $3.8B RealPage cost estimate (cited secondarily in Washington Post 2025) -- not independently read; treat as plausible-strong, not confirmed.]`
- `[RESEARCH GAP: Whether the RealPage settlement will produce measurable rent reductions and over what time horizon -- settlement is compliance-oriented, not outcome-oriented (PP-U03).]`
- `[RESEARCH GAP: FTC and CFPB enforcement posture on surveillance pricing and AI credit guidance under the post-January 2025 administration -- most of the federal enforcement architecture in this case depends on agency willingness to act (PP-U04).]`
- `[RESEARCH GAP: Whether Huskey v. State Farm disparate impact theory survives appellate review -- motion to dismiss survival confirmed; merits outcome unknown (PP-U05).]`
...
- **Insecurity:** When you cannot see the price logic, cannot tell if you are being treated fairly, and cannot exit without loss, you cannot plan. Invisible pricing in essential markets (housing, insurance, credit, healthcare) converts routine transactions into sources of instability.
...
- **Choice:** Portability tools, comparison-shopping infrastructure, and switching cost reductions are proxies for exit in markets where full exit is not realistic. No current mechanism gives consumers comparable pricing logic across providers.
...
- When shopping for insurance or credit, request in writing the specific factors used in your quote or decision. If you receive a generic category rather than a specific factor, that is the basis for a dispute or complaint to your state insurance commissioner or the CFPB.
...
- Switching is costly or time-bound (captivity), so people cannot discipline the market by leaving.
- Appeals are a maze, and the system optimizes denial, delay, or churn.

**Simple test:** if this changes a life outcome (coverage, rent, access, financing), you should get **notice, reason, appeal, records, and a human override**. If you cannot exit, the governance bar must be higher.
